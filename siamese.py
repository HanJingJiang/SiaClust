        ...
        * element = x[element_index]
        ...
        for _ in range(int(num_positive_pairs)):
            ...
            while len(class_list):
                class_1 = random.choice(class_list)
                num_elements = len(class_indices[class_1])
                if num_elements > 1:
                    break
                else:
                    class_list.pop(class_list.index(class_1))
                    continue

            if len(class_list):
                index_1, index_2 = self.__randint_unequal(0, abs(num_elements - 1))  # （0,?）
    ...
    def __create_negative_pairs(self, x, class_indices, num_negative_pairs, num_classes):
        ...
        * element = x[element_index]
        ...
        for _ in range(int(num_negative_pairs)):
            # print('cls_1, len(class_indices[cls_1])', cls_1, len(class_indices[cls_1]) - 1)
            # print('cls_2, len(class_indices[cls_1])', cls_2, len(class_indices[cls_2]) - 1)
            # cls_1, cls_2 = self.__randint_unequal(0, num_classes - 1)  # （0，6）
            while class_list:
                cls_1 = random.choice(class_list)
                if len(class_indices[cls_1]) >= 1:
                    break
                else:
                    class_list.pop(class_list.index(cls_1))
                    continue

            while class_list:
                cls_2 = random.choice(class_list)
                if len(class_indices[cls_2]) > 0 and cls_2 != cls_1:
                    break
                else:
                    if cls_2 != cls_1:class_list.pop(class_list.index(cls_2))
                    continue
